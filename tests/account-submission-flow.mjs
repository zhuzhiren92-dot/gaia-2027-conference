...
try {
  ...
  assert.equal(submission.file_path,null)
  ...
  assert.equal(red,'rgb(180, 35, 24)')
  ...
  console.log('PASS: registration names, welcome/profile, inline editor, draft persistence without upload, validation, submit, own-file download, profile edits, save failure, mobile overflow, no JS errors.')
} finally {
  await context.close()
  await browser.close()
}
